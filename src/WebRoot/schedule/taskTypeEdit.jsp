...
<form id="taskTypeForm" method="get" name="taskTypeForm" action="taskTypeDeal.jsp">
    ...
    <table>
        <tr>
            <td>任务名称:</td>
            <td><input type="text" id="taskType" name="taskType"  <%=editSts%> value="<%=taskType.getBaseTaskType()%>" width="30"></td>
            <td>任务处理的SpringBean:</td>
            <td><input type="text" id="dealBean" name="dealBean" value="<%=taskType.getDealBeanName()%>" width="30"></td>
        </tr>
        <tr>
            <td>心跳频率(秒):</td>
            <td><input type="text" name="heartBeatRate" value="<%=taskType.getHeartBeatRate()/1000.0 %>" width="30"></td>
            <td>假定服务死亡间隔(秒):</td>
            <td><input type="text" name="judgeDeadInterval" value="<%=taskType.getJudgeDeadInterval()/1000.0 %>" width="30"></td>
        </tr>
        <tr>
            <td>线程数：</td>
        ...
        <tr>
            <td>没有数据时休眠时长(秒)：</td>
            <td><input type="text" name="sleepTimeNoData" value="<%=taskType.getSleepTimeNoData()/1000.0%>" width="30"></td>
            <td>每次处理完数据后休眠时间(秒)：</td>
            <td><input type="text" name="sleepTimeInterval" value="<%= taskType.getSleepTimeInterval()/1000.0%>" width="30"></td>
        </tr>
        <tr>
            <td>执行开始时间：</td>
            <td><input type="text" name="permitRunStartTime" value="<%=taskType.getPermitRunStartTime()==null?"":taskType.getPermitRunStartTime()%>" width="30"></td>
            <td>执行结束时间：</td>
            <td><input type="text" name="permitRunEndTime" value="<%=taskType.getPermitRunEndTime()==null?"":taskType.getPermitRunEndTime()%>" width="30"></td>
        </tr>
        <tr>
            <td>单线程组最大任务项：</td>
            <td><input type="text" name="maxTaskItemsOfOneThreadGroup" value="<%=taskType.getMaxTaskItemsOfOneThreadGroup()%>" width="30"></td>
            <td colspan="2">每一组线程能分配的最大任务数量，避免在随着机器的减少把正常的服务器压死，0或者空表示不限制</td>
        </tr>
        <tr>
            <td>自定义参数(字符串):</td>
            <td colspan="3"><input type="text" id="taskParameter" name="taskParameter" value="<%=taskType.getTaskParameter()==null?"":taskType.getTaskParameter()%>" style="width:657"></td>
        </tr>
        <tr>
            <td>任务项(","分隔):</td>
            <td colspan="3"><TEXTAREA type="textarea" rows="5" , id="taskItems" name="taskItems" style="width:657"><%=taskItems%> </TEXTAREA></td>
        </tr>
